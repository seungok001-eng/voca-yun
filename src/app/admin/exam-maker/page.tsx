...
const KIND_INFO: Record<Kind, { label: string; icon: string; desc: string; want: "WORDS" | "TEXT" }> = {
  VOCAB: { label: "단어 시험", icon: "🔤", desc: "단어와 뜻이 적힌 사진·PDF·엑셀을 올리세요.", want: "WORDS" },
...
const VIEW_KO: Record<View, string> = { student: "📄 문제지", answer: "✅ 정답지", explain: "📚 해설지" };
const CIRCLED = ["①", "②", "③", "④", "⑤"];
...
.e-exp-q { font-weight:700; color:#16204a; white-space:pre-wrap; }
.e-exp-a { color:#0a7d43; font-weight:700; margin:3px 0; }
.e-exp-b { color:#333; line-height:1.7; white-space:pre-wrap; }
...
// ── 시험지 한 장 (문제지 / 정답지 / 해설지) ─────────────────────────
// 미리보기와 인쇄, 여러 장 한꺼번에 인쇄에서 모두 이 컴포넌트를 쓴다.
function PaperSheet({ paper, view }: { paper: Paper; view: View }) {
  const org = paper.orgName ?? "정철 VOCA";
  const blanks = (s: string, answers?: boolean) =>
    s.split(/(\{\{\d+\}\})/g).map((part, i) => {
      ...
      return <span className="e-blank" key={i}>({m[1]})</span>;
    });

  if (view === "student") {
    return (
      <div className="esheet">
        <p className="e-org">{org}</p>
        ...
        {paper.passage && <div className="e-passage">{blanks(paper.passage)}</div>}
        ...
        {paper.questions.map((q) => (
          <div className="e-q" key={q.no}>
            ...
            {q.choices && q.choices.length > 0 && (
              <div className="e-choices">{q.choices.map((c, i) => <span key={i}>{CIRCLED[i]} {c}</span>)}</div>
            )}
            {(!q.choices || q.choices.length === 0) && (
              paper.kind === "COMPOSITION" ? <div className="e-write2"><div /><div /></div> : <div className="e-write" />
            )}
          </div>
        ))}
        <p className="e-foot">{org} · 정철 VOCA</p>
      </div>
    );
  }
  ...
  if (view === "answer") {
    return (
      <div className="esheet">
        <p className="e-org">{org} · 교사용</p>
        <h2 className="e-title">{paper.title} — 정답</h2>
        <p className="e-sub">{paper.levelLabel} · {paper.questions.length}문항</p>
        {paper.passage && (
          <>
            <p className="e-ans-title">본문 (정답 표시)</p>
            <div className="e-passage">{blanks(paper.passage, true)}</div>
          </>
        )}
        <p className="e-ans-title">정답</p>
        <div className="e-ans-grid">
          {paper.questions.map((q) => <div key={q.no}><b>{q.no}.</b>{q.answer}</div>)}
        </div>
        <p className="e-foot">{org} · 정철 VOCA</p>
      </div>
    );
    ...
    <div className="esheet">
      ...
      {paper.teacherNotes?.length > 0 && (
        <>
          ...
          {paper.teacherNotes.map((n, i) => <div className="e-note" key={i}><b>{n.heading}</b><p>{n.body}</p></div>)}
        </>
      )}
      {paper.passageKo && (
        <>
        ...
        <>
          ...
          <div className="e-vocab">
            {paper.vocabNotes.map((v, i) => (
              <div key={i}><b>{v.word}</b> — {v.meaning}{v.note && <i>{v.note}</i>}</div>
            ))}
          </div>
        </>
      )}
...
// ── 문항 편집기 ────────────────────────────────────────────────────
function QuestionEditor({ draft, onChange }: { draft: Paper; onChange: (p: Paper) => void }) {
  const isCloze = draft.kind === "CLOZE";
  const setQ = (i: number, patch: Partial<Q>) =>
    onChange({ ...draft, questions: draft.questions.map((q, k) => (k === i ? { ...q, ...patch } : q)) });
  const renumber = (qs: Q[]) => qs.map((q, i) => ({ ...q, no: i + 1 }));
  const move = (i: number, d: -1 | 1) => {
    const qs = [...draft.questions];
    const j = i + d;
    if (j < 0 || j >= qs.length) return;
    [qs[i], qs[j]] = [qs[j], qs[i]];
    onChange({ ...draft, questions: renumber(qs) });
  };
  const remove = (i: number) => {
    if (!confirm(`${i + 1}번 문항을 삭제할까요?`)) return;
    onChange({ ...draft, questions: renumber(draft.questions.filter((_, k) => k !== i)) });
  };
  const add = () =>
    onChange({
      ...draft,
      questions: renumber([...draft.questions, { no: 0, prompt: "", answer: "", explanation: "" }]),
    });

  return (
    <div className="esheet space-y-3" style={{ fontSize: 13 }}>
      <p className="e-org">문항 편집</p>
      <div className="grid gap-2 sm:grid-cols-2">
        <label className="text-xs font-bold text-slate-500">제목
          <input className="input !py-2 mt-1" value={draft.title} onChange={(e) => onChange({ ...draft, title: e.target.value })} />
        </label>
        <label className="text-xs font-bold text-slate-500">시험지 위 지시문
          <input className="input !py-2 mt-1" value={draft.instruction} onChange={(e) => onChange({ ...draft, instruction: e.target.value })} />
        </label>
      </div>
      {isCloze && (
        <p className="text-[11px] text-amber-600">빈칸 자체를 넣고 빼는 것은 '빈칸 편집'에서 하세요. 여기서는 정답·해설만 고칠 수 있습니다.</p>
      )}

      {draft.questions.map((q, i) => (
        <div key={i} className="rounded-xl border border-slate-200 p-3 space-y-2 bg-slate-50/60">
          <div className="flex items-center gap-2">
            <span className="font-black text-[#16204a]">{q.no}.</span>
            <span className="flex-1" />
            {!isCloze && (
              <>
                <button className="chip bg-white text-slate-500 !py-1" onClick={() => move(i, -1)} disabled={i === 0}>▲</button>
                <button className="chip bg-white text-slate-500 !py-1" onClick={() => move(i, 1)} disabled={i === draft.questions.length - 1}>▼</button>
                <button className="chip bg-white text-rose-500 !py-1" onClick={() => remove(i)}>삭제</button>
              </>
            )}
          </div>
          {(draft.kind === "COMPOSITION" || q.promptKo !== undefined) && (
            <label className="block text-[11px] font-bold text-slate-500">학생에게 보이는 우리말
              <input className="input !py-1.5 mt-0.5" value={q.promptKo ?? ""} onChange={(e) => setQ(i, { promptKo: e.target.value })} />
            </label>
          )}
          <label className="block text-[11px] font-bold text-slate-500">문제 {isCloze && "(빈칸이 든 문장)"}
            <textarea className="input !py-1.5 mt-0.5 min-h-[56px]" value={q.prompt} onChange={(e) => setQ(i, { prompt: e.target.value })} />
          </label>
          {!isCloze && (
            <div>
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-bold text-slate-500">보기 (객관식일 때)</span>
                {(q.choices?.length ?? 0) < 5 && (
                  <button className="text-[11px] text-[#16204a] font-bold"
                    onClick={() => setQ(i, { choices: [...(q.choices ?? []), ""] })}>+ 보기 추가</button>
                )}
              </div>
              {(q.choices ?? []).map((c, ci) => (
                <div key={ci} className="flex items-center gap-1.5 mt-1">
                  <span className="text-slate-400 w-5">{CIRCLED[ci]}</span>
                  <input className="input !py-1.5 flex-1" value={c}
                    onChange={(e) => setQ(i, { choices: q.choices!.map((x, k) => (k === ci ? e.target.value : x)) })} />
                  <button className="text-slate-300 hover:text-rose-500 px-1"
                    onClick={() => setQ(i, { choices: q.choices!.filter((_, k) => k !== ci) })}>✕</button>
                </div>
              ))}
            </div>
          )}
          <label className="block text-[11px] font-bold text-slate-500">정답
            <input className="input !py-1.5 mt-0.5" value={q.answer} onChange={(e) => setQ(i, { answer: e.target.value })} />
          </label>
          <label className="block text-[11px] font-bold text-slate-500">해설
            <textarea className="input !py-1.5 mt-0.5 min-h-[56px]" value={q.explanation} onChange={(e) => setQ(i, { explanation: e.target.value })} />
          </label>
          <label className="block text-[11px] font-bold text-slate-500">핵심 포인트 (쉼표로 구분)
            <input className="input !py-1.5 mt-0.5" value={(q.points ?? []).join(", ")}
              onChange={(e) => setQ(i, { points: e.target.value.split(",").map((s) => s.trim()).filter(Boolean) })} />
          </label>
        </div>
      ))}
      {!isCloze && (
        <button className="btn-ghost w-full !py-2 text-sm" onClick={add}>+ 문항 추가</button>
      )}
    </div>
  );
}
...
export default function ExamMakerPage() {
  ...
  const [show, setShow] = useState<"none" | View | "edit" | "questions">("none");
  ...
  const [printViews, setPrintViews] = useState<Set<View>>(new Set(["student", "answer", "explain"]));
  const [printQueue, setPrintQueue] = useState<{ papers: Paper[]; views: View[] } | null>(null);
  ...
  // 저장 (편집 결과) — 저장된 시험지면 서버에도 반영
  async function persist(next: Paper) {
    if (next.id) await api("/api/admin/exam/saved", { method: "PUT", body: JSON.stringify({ id: next.id, paper: next }) });
    setPaper(next);
    loadSaved();
  }
      ...
      const cleaned: Paper = {
        ...
        questions: draft.questions
          .map((q, i) => ({
            ...
            promptKo: q.promptKo?.trim() || undefined,
          }))
          .map((q) => ({ ...q, choices: q.choices?.length ? q.choices : undefined })),
      };
  ...
  async function saveEdit() {
    ...
    try {
      ...
      const questions: Q[] = sorted.map((b, k) => {
        ...
        return {
          no: k + 1, prompt: sentenceOf(edit.tokens, b), answer: answers[k],
          explanation: prev?.explanation ?? "", points: prev?.points,
        };
      });
      ...
      await persist({ ...paper, passage, questions, wordBank });
      setEdit(null);
      setShow("student");
    } catch (e) {
      alert(e instanceof Error ? e.message : "저장하지 못했습니다.");
  ...
  // ── 인쇄 ──
  function openPrintWindow(html: string, name: string) {
    const win = window.open("", "_blank", "width=900,height=1000");
    if (!win) { alert("팝업이 차단됐어요. 팝업 허용 후 다시 눌러주세요."); return; }
    win.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${name}</title><style>${SHEET_CSS}</style></head><body>${html}</body></html>`);
    win.document.close();
    win.focus();
  ...
  function printCurrent() {
    if (sheetRef.current && paper) openPrintWindow(sheetRef.current.innerHTML, paper.title);
  }

  // 여러 장 한꺼번에: 숨긴 영역에 모두 그린 뒤 그 HTML을 인쇄창으로 보낸다
  useEffect(() => {
    if (!printQueue || !printRef.current) return;
    const html = printRef.current.innerHTML;
    setPrintQueue(null);
    setPrinting(false);
    if (html.trim()) openPrintWindow(html, `시험지 ${printQueue.papers.length}장`);
  }, [printQueue]);

  function printPaperAll() {
    if (!paper) return;
    const views = (["student", "answer", "explain"] as View[]).filter((v) => printViews.has(v));
    if (views.length === 0) { alert("인쇄할 장을 하나 이상 고르세요."); return; }
    setPrinting(true);
    setPrintQueue({ papers: [paper], views });
  }

  async function printPicked() {
    const ids = saved.filter((r) => picked.has(r.id)).map((r) => r.id);
    const views = (["student", "answer", "explain"] as View[]).filter((v) => printViews.has(v));
    if (ids.length === 0) { alert("인쇄할 시험지를 고르세요."); return; }
    if (views.length === 0) { alert("인쇄할 장을 하나 이상 고르세요."); return; }
    setPrinting(true);
    try {
      const papers = await Promise.all(ids.map((id) => api<Paper>(`/api/admin/exam/saved?id=${id}`)));
      setPrintQueue({ papers, views });
    } catch (e) {
      setPrinting(false);
      alert(e instanceof Error ? e.message : "시험지를 불러오지 못했습니다.");
    }
  }
  ...
  const toggleView = (v: View) =>
    setPrintViews((s) => { const n = new Set(s); if (n.has(v)) n.delete(v); else n.add(v); return n; });
  ...
  const ViewPicker = () => (
    <div className="flex flex-wrap items-center gap-1.5">
      {(["student", "answer", "explain"] as View[]).map((v) => (
        <label key={v} className={"chip cursor-pointer !py-1.5 " + (printViews.has(v) ? "bg-[#16204a] text-white" : "bg-slate-100 text-slate-500")}>
          <input type="checkbox" className="hidden" checked={printViews.has(v)} onChange={() => toggleView(v)} />
          {VIEW_KO[v]}
        </label>
      ))}
    </div>
    ...
    <div className="space-y-4">
      ...
      <div className="card p-5 space-y-4">
              ...
              {KIND_INFO[kind].want === "WORDS" ? (
                words.length > 0 ? (
                  <div className="rounded-xl border border-slate-200 max-h-72 overflow-y-auto">
                    ...
                    {words.map((w, i) => (
                      <div key={i} className="flex items-center gap-2 px-3 py-1.5 border-t border-slate-100">
                        ...
                        <button className="text-slate-300 hover:text-rose-500 px-1" onClick={() => setWords((ws) => ws.filter((_, k) => k !== i))}>✕</button>
                      </div>
                    ))}
                  ...
                  <p className="text-sm text-slate-400 rounded-xl border border-dashed border-slate-200 p-6 text-center">
                    사진·PDF·엑셀을 올리면 여기에 단어 목록이 나옵니다.
                  </p>
                )
              ) : (
                <textarea className="input min-h-[180px] font-mono text-[13px] leading-relaxed"
                  placeholder={kind === "CLOZE" ? "독해 본문을 붙여넣거나 사진·PDF를 올리세요." : "우리말 문장이나 영어 문장을 붙여넣거나 사진·PDF를 올리세요. 둘 다 있어도 됩니다."}
        ...
        <div className="flex flex-wrap items-center gap-3 pt-1">
          <input className="input flex-1 min-w-[220px]" placeholder="시험지 제목 (비우면 자동)" value={title} onChange={(e) => setTitle(e.target.value)} />
      ...
      {paper && (
        <div className="card p-4 space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-2 border-t border-slate-100 pt-3">
            ...
            <ViewPicker />
            <button className="chip bg-[#c9a227] text-white !py-2 !px-4 font-black ml-auto" disabled={printing} onClick={printPaperAll}>
              {printing ? "준비 중..." : "🖨️ 고른 장 모두 인쇄"}
            </button>
          </div>
        </div>
      )}
      ...
      {/* 미리보기 · 편집 · 인쇄 */}
      {paper && (
        <div className={show !== "none" ? "fixed inset-0 z-50 bg-black/50 overflow-y-auto p-3 sm:p-8" : "hidden"} onClick={() => setShow("none")}>
          <div className="max-w-[860px] mx-auto" onClick={(e) => e.stopPropagation()}>
              ...
              {show === "edit" ? (
                <>
                  <span className="text-white text-sm font-bold self-center mr-auto">
                    ✏️ 낱말을 누르면 빈칸이 되고, 빈칸을 누르면 낱말로 돌아옵니다 · 빈칸 {edit?.blanks.length ?? 0}개
                  ...
                  <button className="chip bg-white text-slate-600 !py-2 !px-4" disabled={saving} onClick={() => { setEdit(null); setShow("student"); }}>취소</button>
                </>
              ) : show === "questions" ? (
                <>
                  <span className="text-white text-sm font-bold self-center mr-auto">📝 문항을 고친 뒤 저장하세요</span>
                  <button className="chip bg-[#c9a227] text-white !py-2 !px-4 font-black" disabled={saving} onClick={saveQuestions}>
                    {saving ? "저장 중..." : "💾 저장"}
                  </button>
                  <button className="chip bg-white text-slate-600 !py-2 !px-4" disabled={saving} onClick={() => { setDraft(null); setShow("student"); }}>취소</button>
                </>
              ) : (
                <>
                  {(["student", "answer", "explain"] as View[]).map((v) => (
            ...
            <div className="rounded-xl overflow-hidden shadow-2xl" ref={sheetRef}>
              {show === "questions" && draft && <QuestionEditor draft={draft} onChange={setDraft} />}
              {show === "edit" && edit && (
                <div className="esheet">
                  ...
                  <div className="ed-passage">
                    {edit.tokens.map((t, i) => {
                      ...
                      return (
                        <span key={i}>
                          {t.lead}
                          {t.core ? <span className="ed-w" title="누르면 빈칸이 됩니다" onClick={() => toggleToken(i)}>{t.core}</span> : null}
                          {t.trail}{t.ws}
                        </span>
                      );
                    })}
              ...
              {(show === "student" || show === "answer" || show === "explain") && <PaperSheet paper={paper} view={show} />}
            </div>
          </div>
        </div>
      )}
